...
#define viscos_val 0.08 //粘度系数

#define input_data false
#define virtual_part true

struct str {
    int test_step; //用于调试程序
    int ntotal;
    int part_ntotal; //记录门左边粒子数,方便初始化后面的密度，因为高度不一样
    int n_gate;
    int nvirt;
    int ncases;        //记录越界粒子数量
    int index_tensile; //拉伸校正，Fortran位于kernel中,1：使用，0：不使用
    int i_MLS_part, i_MLS_step;
    double wab, frx, fry, frxi, frxj, fryi, fryj; //核函数相关值
    double aa1, aa2, aaa, adh, aa24, cc1, cc2, dd1;
    double deltap, wdeltap, od_wdeltap;
    double new_dt;
    double time;
    double B, cs0;
    int nstep_DBC;
    ...
    double pr[tot_maxn];
    double rho[tot_maxn];
    double rho_sum_MLS[tot_maxn];
    double A_MLS[tot_maxn][3][3];
    double beta0_MLS[tot_maxn];
    double beta1x_MLS[tot_maxn];
    double beta1y_MLS[tot_maxn];
    double A_LU[3][3];
    double A_MLS_inverse[3][3];
    double b_input_vector[3];
    double x_solution[3];
