...
int main()
{
    ...
    gate(vari);      //获取gate粒子的位置及gate粒子的初始数据
    ...
    getdata(vari);
    ini_divide(vari, 0);
    divide(vari, vari->ntotal + vari->n_gate, vari->ntotal + vari->nvirt + vari->n_gate, 0);
    keep_list(vari);

    //主循环
    while (vari->time < tmax) {
        vari->visc_dt = 0.0;
        //设置每0.01s输出粒子属性相关信息到文件，用于可视化out=0.02,ngrab表示输出文件数量
        if (vari->time - outt * ngrab >= outt) {
            output = 1;
            ngrab += 1;
        } else {
            output = 0;
        }
        vari->test_step = timestep;
        single_step(vari);
        vari->time += vari->new_dt;
        timestep += 1;
        // if (timestep % 30 == 0) {
        ...
        if (output == 1) {
            writevtk(vari, ngrab, 1, 1); //ngrab用于文件的编号,1表示输出虚粒子一起,0表示不含虚粒子输出
            std::cout << "time=" << vari->time << "  "
                      << "dt=" << vari->new_dt << "  "
