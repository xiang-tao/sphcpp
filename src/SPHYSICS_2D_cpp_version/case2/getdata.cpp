...
void getdata(str* vari)
{
    ...
    vari->ddt_c = vari->new_dt;
    vari->ddt_p = vari->new_dt;

    // Cubic spline kernel function
    vari->aa1 = 10.0 / (7.0 * pi);
    vari->aa2 = vari->aa1 / vari->h2;
    vari->aaa = vari->aa1 / (vari->h2 * vari->hsml);
    vari->adh = vari->aa2;
    vari->aa24 = 0.25 * vari->aa2;
    vari->cc1 = -3.0 * vari->aaa;
    vari->dd1 = 9.0 * vari->aaa / 4.0;
    vari->cc2 = -3.0 * vari->aaa / 4.0;
    vari->deltap = 1.0 / 1.5;
    vari->wdeltap = vari->aa2 * (1.0 - 1.5 * vari->deltap * vari->deltap + 0.75 * vari->deltap * vari->deltap * vari->deltap);
    vari->od_wdeltap = 1.0 / vari->wdeltap;

    vari->eta = 0.1 * vari->hsml;
    vari->eta2 = vari->eta * vari->eta;

    int tol = vari->ntotal;
    vari->xmin = vari->x[0][tol];
    vari->xmax = vari->x[0][tol];
    vari->ymin = vari->x[1][tol];
    vari->ymax = vari->x[1][tol];

    //找出横坐标纵坐标中虚粒子的最大最小值，便于生成列表,后续用于控制整体粒子的范围，比如由于飞溅.
    for (int i = tol + 1; i < tol + vari->n_gate + vari->nvirt; i++) {
    ...
    // std::cout << "xmax_container=" << vari->xmax_container << std::endl;
    // std::cout << "xmin_container=" << vari->xmin_container << std::endl;
    // std::cout << "ymin_container=" << vari->ymin_container << std::endl;
    //前面比较了虚粒子和门粒子，现在比较流动粒子的范围最值，相当于是初始时候所有粒子的范围情况，便于生成列表
    for (int i = 0; i < tol; i++) {
        if (vari->x[0][i] > vari->xmax) {
            vari->xmax = vari->x[0][i];
        }
    ...
    vari->ycontrol = vari->ymin + vari->ncy * 2.0 * vari->hsml; //ycontrol>=ymax
